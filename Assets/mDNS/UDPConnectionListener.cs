...
namespace Toolkit.NetworkUtilites
{
    public interface IUDPConnectionEventListener
    {
        public class EventData
        {
            ...
            public EventData(Type eventType, string name, IPAddress ipAdress, int port)
            {
                EventType = eventType;
                Name = name;
                IP = ipAdress;
    ...
    public class UDPConnectionListener : MonoBehaviour
    {
        ...
        private const int listenPort = 8081; // The port number to listen on
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            ...
            Debug.Log("Received UDP message from " + remoteEndPoint + ": " + receivedString);
        ...
        private void OnConnectedToQuadruped(string name, IPAddress ip, int port)
        {
          //  var quadrupedData = new UDPConnectionData(name, ip, port);
          NotifyListeners(new IUDPConnectionEventListener.EventData(IUDPConnectionEventListener.Type.OnRobotConnected,name,ip,port));
        }
